...
export function createSimEngine({workers=Math.max(1,Math.min(3,(navigator.hardwareConcurrency||2)-1))}={}){
 ...
 const params={freq:4,gain:1,tgc:Array(8).fill(50),dynamicRange:52,focus:.55,beating:true,valves:true,color:{on:false,nyquist:.7,roi:{center:0,half:20,r0:.25,r1:.8}}};
 ...
 function viewKey(pose,p){return JSON.stringify([pose.origin,pose.u,pose.d,pose.depth,pose.sector,p.freq,p.gain,p.tgc,p.dynamicRange,p.focus,p.beating,p.valves,p.color.on?p.color:0,variantSeq])}
 // Show `pose`. `phase` = current cardiac phase (0..1); `quick` = lower resolution while dragging.
 function show(pose,phase=0,{quick=false}={}){
  const p={...params,lines:quick?112:168,samples:quick?Math.round(pose.depth/.00036):undefined};
