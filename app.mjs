...
import {scanYSign,screenToPlane,CHEST_Y,CHEST_X,placeAtlasPositions,BODY,setBodyScale,physical} from './geometry.mjs';
...
function drawSim(){
 ...
 ctx.fillStyle='#63ddc8';ctx.beginPath();ctx.arc(cx+Math.sin(rad(p.sector/2))*20,cy+ySign*Math.cos(rad(p.sector/2))*20,3,0,Math.PI*2);ctx.fill();
 ctx.font='10px Segoe UI';ctx.fillStyle='#95a7b4';ctx.textAlign='right';ctx.fillText('MARCADOR →',w-10,16);ctx.textAlign='left';ctx.fillText(`ECO SIMULADO · ${display.freq.toFixed(1).replace('.',',')} MHz · ${patient.name.toUpperCase()}`,10,16);
 for(const [depth,label] of depthTicks(p.depth)){const x=cx+depth*scale*Math.sin(rad(p.sector/2)),y=cy+ySign*depth*scale*Math.cos(rad(p.sector/2));ctx.textAlign='left';ctx.fillText(label,x+5,y)}
...
const LIMITS={x:[-.11,.11],z:[-.155,.2],tilt:[-60,60],rock:[-50,50],depth:[.03,.24],gain:[.3,2.5],freq:[2,12]};
