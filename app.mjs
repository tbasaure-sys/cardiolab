...
import {scanYSign,screenToPlane,CHEST_Y,CHEST_X,placeAtlasPositions} from './geometry.mjs';
...
import {createSimEngine} from './sim-engine.mjs';
import {scanConvert,COLOR_MAP} from './bmode.mjs';
import {VIEW_INFO,markerClock} from './views.mjs';
...
let tutor=null,pediatricDisplay=true,instrument=null,references=null,videoImage=null,echoSource='sim',resumeAfterFreeze=false;const display=initialDisplay();
// simulated B-mode: engine, cardiac phase, cine cache of scan-converted images
const engine=createSimEngine();let phase=0,beatSpeed=1,heartRate=90,lastTick=performance.now(),lastPoseChange=0,simQuick=false,simViews=null,coach=null,chd=null,targetPose=null,lesion=null,overlayContours=false;
...
function makeControl(container,key,label,min,max,step,unit,factor=1,help=''){
  ...
  const change=e=>{lastInputAt=performance.now();if(!ready||(frozen&&!(realEcho()&&key==='gain'))||(realEcho()&&['depth','sector'].includes(key)))return;const v=Number(e.target.value);if(!Number.isFinite(v))return;state[key]=clamp(v,min,max)/factor;if(['tilt','rock','rotation','x','z'].includes(key))stopSweep();syncControls();if(realEcho()&&key==='gain')renderScan();else requestSlice()};
...
makeControl('pose-controls','rotation','Rotar',-180,180,1,'°',1,'Gira el marcador alrededor del haz');
makeControl('pose-controls','tilt','Inclinar · tilt',-60,60,1,'°',1,'Cambia el plano fuera del corte');
makeControl('pose-controls','rock','Bascular · rock',-50,50,1,'°',1,'Orienta el haz dentro del corte');
makeControl('image-controls','depth','Profundidad',80,240,5,'mm',1000);
...
function makeView(id,isPatient){
  ...
  if(isPatient){
    ...
    for(let r=0;r<=rings;r++){const z=-.205+r*.49/rings;for(let i=0;i<sides;i++){const angle=2*Math.PI*i/sides;positions.push(CHEST_X+chestRadius(z)*Math.cos(angle),CHEST_Y+chestDepth(z)*Math.sin(angle),z)}}
...
function setLesionMarker(pos){lesion=pos;for(const v of views){v.lesionMark.visible=!!pos;if(pos)v.lesionMark.position.set(...pos)}}
...
function updateView(v,pose){
  ...
  v.probe.position.set(...pose.origin);const matrix=new THREE.Matrix4().makeBasis(new THREE.Vector3(...pose.u),new THREE.Vector3(...pose.d).negate(),new THREE.Vector3(...pose.n).negate());v.probe.quaternion.setFromRotationMatrix(matrix);
...
function bindProbeDrag(view){
  ...
  function move(event){if(!dragging||frozen||!ready)return;const rect=canvas.getBoundingClientRect();raycaster.setFromCamera(new THREE.Vector2((event.clientX-rect.left)/rect.width*2-1,-(event.clientY-rect.top)/rect.height*2+1),view.camera);
    const hits=raycaster.intersectObject(view.torso).filter(h=>h.point.y<0);if(!hits.length)return;const point=hits[0].point;state.x=clamp(point.x,-.110,.110);state.z=clamp(point.z,-.155,.2);stopSweep();syncControls();requestSlice()}
...
function syncEngine(){engine.set({gain:state.gain,freq:display.freq,tgc:[...display.tgc]})}
...
function accept(msg){
  ...
  if(msg.id===revision){if(active&&JSON.stringify(active.pose)!==JSON.stringify(msg.pose)){crosshair=null;display.points=[];}active=msg;active.visible=msg.cuts.filter(c=>visibleCut(c,msg.pose));active.elapsed=performance.now()-latestRequestedAt;latencies.push(active.elapsed);if(latencies.length>100)latencies.shift();
    ...
    status(frozen?'Plano congelado':(sweeping?'Barrido de sonda':'Vistas sincronizadas'));$('marker-clock').textContent=`Marcador ${Math.round(markerClock(msg.pose.u))||12} h`;coach?.update();$('depth-readout').textContent=realEcho()?'Vídeo':(msg.pose.depth*100).toFixed(0)+' cm';
...
function renderScan(){
  ...
  $('depth-readout').textContent=(active.pose.depth*100).toFixed(0)+' cm';
  ...
  ctx.strokeStyle='#20313c';ctx.lineWidth=.5;ctx.setLineDash([2,5]);for(let depth=.05;depth<=p.depth;depth+=.05){ctx.beginPath();ctx.arc(cx,cy,depth*scale,0,Math.PI*2);ctx.stroke()}ctx.setLineDash([]);
  ...
  for(let depth=.05;depth<=p.depth;depth+=.05){const x=cx+depth*scale*Math.sin(rad(p.sector/2)),y=cy+ySign*depth*scale*Math.cos(rad(p.sector/2));ctx.textAlign='left';ctx.fillText((depth*100).toFixed(0),x+5,y)}
...
function drawSim(){
 ...
 const frame=engine.frameFor(phase);
 if(frame){const key=`${scan.width}x${scan.height}|${cx.toFixed(2)}|${cy.toFixed(2)}|${scale.toFixed(3)}|${ySign}|${display.contrast}`;let e=scanCache.get(frame);
  if(!e||e.key!==key){const img=ctx.createImageData(scan.width,scan.height);scanConvert(frame,img,projection,ratio);if(display.contrast!==1)applyContrast(img,display.contrast);e={key,img};scanCache.set(frame,e)}ctx.putImageData(e.img,0,0)}
 ...
 ctx.fillStyle='#63ddc8';ctx.beginPath();ctx.arc(cx+Math.sin(rad(p.sector/2))*20,cy+ySign*Math.cos(rad(p.sector/2))*20,3,0,Math.PI*2);ctx.fill();
 ctx.font='10px Segoe UI';ctx.fillStyle='#95a7b4';ctx.textAlign='right';ctx.fillText('MARCADOR →',w-10,16);ctx.textAlign='left';ctx.fillText(`ECO SIMULADO · ${display.freq.toFixed(1).replace('.',',')} MHz`,10,16);
 for(let depth=.05;depth<=p.depth;depth+=.05){const x=cx+depth*scale*Math.sin(rad(p.sector/2)),y=cy+ySign*depth*scale*Math.cos(rad(p.sector/2));ctx.fillText((depth*100).toFixed(0),x+5,y)}
 ctx.font='10px Consolas';ctx.fillStyle='#8aa1ae';ctx.fillText(`T ${state.tilt.toFixed(0)}°  R ${state.rock.toFixed(0)}°  ROT ${state.rotation.toFixed(0)}°  Z ${display.zoom.toFixed(1)}×`,12,dop.mode?hImg-8:h-48);
...
let specCanvas=null;
function dopSpec(){const p=poseFromState(state);return {pose:p,theta:dop.theta,gateDepth:dop.depth*p.depth,gateLen:.003,mode:dop.mode,freq:display.freq,scale:dop.mode==='cw'?(dop.scale||4):dop.scale,baseline:dop.baseline,gain:1}}
let dopTimer=0;
const specKey=spec=>JSON.stringify([spec.pose.origin,spec.pose.u,spec.pose.d,spec.theta,spec.gateDepth,spec.mode,spec.freq,spec.scale,spec.baseline,chdVariantKey]);
...
async function cwOnLesion(id){const plan=CW_PLAN[id];let pt=lesionPosition(id);if(!plan||!pt||!ready)return;
 ...
 const target=cwTarget(id);if(target){const {solveState}=await import('./views.mjs');const sol=solveState(target,PRESET_OF_WINDOW[VIEW_INFO[plan.view].window]);applyState({...sol.state,depth:target.depth,mode:'echo'})}
...
for(const mode of ['anatomy','echo'])$(`mode-${mode}`).onclick=()=>{if(frozen)return;state.mode=mode;$('echo-tools').hidden=mode!=='echo';for(const m of ['anatomy','echo'])$(`mode-${m}`).classList.toggle('active',m===mode);if(!realEcho())references?.video.pause();syncControls();requestSlice()};
...
<p><b>Imagen eco simulada.</b> Las mallas se convierten en un volumen de tejidos de 1 mm (miocardio, cavidades, válvulas, grandes vasos, pericardio, costillas) y un volumen torácico de 2,5 mm. Pulmón, hígado, diafragma y la vena cava inferior abdominal con las venas suprahepáticas son aproximaciones propias, porque el atlas no los incluye. El esternón y el xifoides se tratan como cartílago con núcleos de osificación, como en el niño: atenúan pero no bloquean. Las costillas quedan registradas con el corazón del atlas y la piel ~1 cm por delante del esternón. Cada línea del sector se traza con atenuación dependiente de la frecuencia, reflexión especular según el ángulo, speckle fijado a la anatomía, sombra de costillas y pulmón, reverberaciones pleurales y la resolución del haz. Para acercarla a un equipo real se añaden heterogeneidad del tejido (agrupación del speckle y bordes irregulares), directividad del elemento, un velo de lóbulos laterales, ruido de campo cercano, reducción de speckle y un mapa de grises con rechazo. Es una simulación simplificada: no hay armónicos ni valores medibles con significado clínico.</p>
...
function explainConsole(mode){const texts={M:'Modo M registra la posición a lo largo de una línea frente al tiempo. Todavía no está implementado en el simulador; úsalo como concepto.',PWinfo:'PW: mide velocidades en un volumen de muestra a una profundidad. Tiene un límite (Nyquist) que baja cuanto más profundo está el volumen: por encima aparece aliasing.',CWinfo:'CW: registra todas las velocidades a lo largo de la línea, sin aliasing, pero sin saber a qué profundidad está cada una. Es el modo para medir jets rápidos (CIV, estenosis, insuficiencia tricuspídea): ΔP = 4·v².',Color:'Doppler color: rojo es flujo hacia el transductor y azul, flujo que se aleja; solo se mide la componente paralela al haz. Si la velocidad supera la escala (límite de Nyquist) aparece aliasing, y los jets turbulentos se ven en mosaico. Actívalo con la tecla Color.',PW:'Doppler pulsado permite situar una muestra a una profundidad. No hay señal espectral adquirida ni flujo calculado en este atlas.',CW:'Doppler continuo interroga movimiento a lo largo del haz sin localizar una muestra única en profundidad. No está simulado en este motor.',controls:'Arrastra las perillas hacia arriba o abajo; también aceptan rueda, flechas y un valor numérico. TGC modifica la intensidad por profundidad en el corte sintético y por bandas de imagen en el vídeo. Sobre un clip exportado los ajustes son digitales; no recuperan señal cruda, tejido fuera del encuadre ni información perdida. La orientación del vídeo se conserva. Zoom y contraste son ajustes de presentación. La esfera mueve un cursor sobre el plano 2D y su punto en 3D. Cáliper permite fijar dos puntos del corte: mide distancia geométrica del atlas, no una medida clínica calibrada. Congelar detiene la sonda y pausa el clip si está en Eco; se mantienen disponibles los ajustes de presentación. Restablecer imagen recupera valores docentes, no aplica optimización clínica.'};openInfo(mode==='controls'?'Consola: controles y perillas':`${mode} · concepto`, `<p>${texts[mode]}</p><p>Consola docente: no reproduce el software ni la calibración de un equipo comercial.</p>`)}
// cockpit layout and keyboard: probe nudges go through the same state as the sliders
const LIMITS={x:[-.11,.11],z:[-.155,.2],tilt:[-60,60],rock:[-50,50],depth:[.08,.24],gain:[.3,2.5],freq:[2,8]};
function nudgeProbe(key,delta){if(!ready||frozen)return;stopSweep();let v=state[key]+delta;if(key==='rotation')v=((v+540)%360)-180;else v=clamp(v,...LIMITS[key]);state[key]=v;syncControls();requestSlice()}
function consoleValue(key){return key==='freq'?display.freq:state[key]}
...
const panels={coach:$('coach'),chd:$('chd')};let openPanel=null;
function showPanel(name){
 ...
 if(leaving==='coach'&&openPanel!=='coach')coach?.stop();if(leaving==='chd'&&openPanel!=='chd')chd?.leave();if(openPanel==='chd')chd?.enter();
 $('practice-shell').classList.toggle('with-tutor',!!openPanel||!$('tutor').hidden);
}
for(const k of Object.keys(panels))$('tab-'+k).onclick=()=>{if(!coach||!chd){status('Preparando el simulador…');return}showPanel(k)};
$('course-toggle').addEventListener('click',()=>{if(openPanel){for(const el of Object.values(panels))el.hidden=true;for(const k of Object.keys(panels))$('tab-'+k).classList.remove('active');if(openPanel==='coach')coach?.stop();if(openPanel==='chd')chd?.leave();openPanel=null}},true);
...
engine.ready.then(async()=>{
 ...
 coach=mountCoach({...common,host:$('coach'),setTarget:setTargetPose,ySign:w=>scanYSign(PRESET_OF_WINDOW[w]||'plax',pediatricDisplay)});
